...
import org.junit.jupiter.api.Test
	...
	class InputHandlerTest {
		...
		// validateCarNames
		// FEATURE TESTS TODO: implement parsing unit test
		@Test
		...
		fun `exception when list is empty`() {
			assertThrows<IllegalArgumentException> {
				InputHandler.validateCarNames(listOf(""))
			}
		}
		...
		fun `exception when only one car name is provided`() {
			assertThrows<IllegalArgumentException> {
				InputHandler.validateCarNames(listOf("car1"))
			}
		}
		...
		@Test //  TODO: Make validateCarNames() handle duplicates assuming names are already lowercased by a function called parseInput()
		...
		@Test // TODO: validateCarNames() - implement regex to allow only letters and numbers, to avoid shell-sensitive characters and names being unreadable(i.e. being writen only with symbols)
		fun `exception when car names use unreadable or shell sensitive characters`() {
			assertThrows<IllegalArgumentException> {
				InputHandler.validateCarNames(listOf("car?", "car1" ))
			}
		}

		// Added to prevent UI overload during race simulation
		@Test // TODO: validateCarNames() - add a limit to the number of car names to avoid overloading the system
		...
		fun `exception when totalRounds is zero`() {
			assertThrows<IllegalArgumentException> {
				InputHandler.validateTotalRounds(0)
			}
		}
		...
		fun `exception when totalRounds exceeds system-safe limit of 10`() {
			assertThrows<IllegalArgumentException> {
				InputHandler.validateTotalRounds(11)
			}
		}
	}
